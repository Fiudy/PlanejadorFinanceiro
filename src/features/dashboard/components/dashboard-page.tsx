...
import { ArrowDownRight, ArrowUpRight, BarChart3, ChevronLeft, ChevronRight, Equal, FileUp, ShieldCheck } from "lucide-react";
...
export function DashboardPage() {
  ...
  return <div>
    <div className="oc-content-head"><div className="oc-month-control"><button onClick={() => move(-1)}><ChevronLeft size={18}/></button><div className="oc-month-label"><small>Período</small><b>{month.toLocaleDateString("pt-BR",{month:"long",year:"numeric"})}</b></div><button onClick={() => move(1)}><ChevronRight size={18}/></button><button className="!w-auto px-3 text-xs font-bold" onClick={() => setMonth(new Date())}>Hoje</button></div><div className="flex items-center gap-3"><span>{list.length} lançamentos no período</span><button className="oc-soft" onClick={() => setImportOpen(true)}><FileUp size={16}/>Importar fatura ou extrato</button></div></div>
